...
unsigned __stdcall window_thread(void* pdata) {
	...
	ImGui_ImplWin32_Init(pwindow->win_handle);
	//ImGui_ImplDX10_Init(g_pd3dDevice);
	ASGI::Init(ASGI::GIType::GI_VULKAN);
	...
	pwindow->pSwapchain = ASGI::CreateSwapchain(swapchain_create_info);
	if (pwindow->pSwapchain == nullptr) {
		return 0;
	}
	pwindow->pExcuteQueue = ASGI::AcquireExcuteQueue(ASGI::QueueType::QUEUE_TYPE_GRAPHICS);
	if (pwindow->pExcuteQueue == nullptr) {
		return 0;
	}
	...
	while (true)
	{
		...
		pwindow->Render();
		//
		auto swapChain = pwindow->pSwapchain.get();
		ASGI::Present(pwindow->pExcuteQueue, 1, &swapChain);
		ASGI::WaitQueueExcuteFinished(1, &pwindow->pExcuteQueue);
	}
	//
